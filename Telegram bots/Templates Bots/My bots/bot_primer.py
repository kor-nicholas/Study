...
import smtplib # отправка на почту

# 1259568399:AAH9a-2E3LKyVHldr73wxuuObA404M53EvM - test_145_bot

bot = telebot.TeleBot('1119755573:AAGqWzU-sBh50yKYo3KSDrEpQciwgDsmCw8')
...
def start(message):
    ...
    murkup_inline = types.InlineKeyboardMarkup() # создаем кавиатуру под сообщением
...
# bot.register_next_step_handler(message, function_name) - запуск другой функции

# user_id = message.from_user.id # = chat_id
# group_id = bot.get_chat("@channel_name").id


"""     КНОПКИ
Reply - в клавиатуоре ; Inline - под сообщением

from telebot import  types

@bot.message_handler(commands=['inline'])
def inline(message):
    markup_inline = types.InlineKeyboardMarkup() # клавиатура
    btn1 = types.InlineKeyboardButton(text="текст на кнопке 1", callback_data="if_btn1")
    btn2 = types.InlineKeyboardButton(text="текст на кнопке 2", callback_data="if_btn2")

    markup_inline.add(btn1,btn2) # добавляем на клавиатуру кнопки
    bot.send_message(message.chat.id, "Кнопки : ", reply_markup=markup_inline) # добавляем клавиатуру с кнопками к сообщению

# обработчик Inline клавиатуры
@bot.callback_query_handler(func = lambda call : True)
def answer(call):
    if call.data == 'if_btn1':
        bot.send_message(call.message.chat.id, "Текст") # важно - call.message.chat.id
    elif call.data == 'if_btn2':
        pass

@bot.message_handler(commands=['reply'])
def reply(message):
    markup_reply = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    # resize - чтобы кнопки клавиатуры были не огромными
    # one time - чтобы после нажатия, клавиатура с кнопками убралась, но была доступна (при нажатии на открытие кнопок)
    btn = types.KeyboardButton('Текст кнопки')

    markup_reply.add(btn)
    bot.send_message(message.chat.id, "Текст, после которого будет добавлена клавиатура кнопок", reply_markup=markup_reply)

# Обработчик Reply клавиатуры - обработчик текстом
@bot.message_handler(content_types=['text'])
def reply(message):
        ...
        for i in statuss:
            if i == bot.get_chat_member(chat_id=my_channel_id, user_id=message.from_user.id).status:
                bot.send_sticker(message.chat.id,
                                 "CAACAgIAAxkBAAEBAlVfAc_5RxAVtkCserEzRwiwmh0UAwACPAAD-7g6BAwMRWBCpy3SGgQ")
                break
        else:
