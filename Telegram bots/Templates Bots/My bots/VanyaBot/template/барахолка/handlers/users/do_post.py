...
async def get_post_photo(message: types.Message, state: FSMContext):
    ...
    await bot.send_photo(chat_id=message.from_user.id, photo=photo_file, caption=f'{post}\n'
                                                                                 f'➖➖➖➖➖➖➖➖➖➖➖➖\n'
                                                                                 f'Отправить?',
...
async def do_post(call: types.CallbackQuery, callback_data: dict):
    ...
    if name == 'yes':
        moderate_post_menu = InlineKeyboardMarkup(row_width=3,
                                                  inline_keyboard=[
                                                      [
                                                          InlineKeyboardButton(
                                                              text="Опубликовать",
                                                              callback_data=moderate_post.new(name='yes',
                                                                                              user_id=call.message.chat.id)
                                                          ),
                                                          InlineKeyboardButton(
                                                              text="Не публиковать",
                                                              callback_data=moderate_post.new(name='change',
                                                                                              user_id=call.message.chat.id)
                                                          ),
                                                      ],
                                                  ])
        for admin in ADMINS:
            ...
            await bot.send_photo(chat_id=int(admin), caption=message,
...
async def do_post(call: types.CallbackQuery, callback_data: dict):
    ...
    if status == 'yes':
        await bot.send_photo(user_id, caption=call.message.caption + '\nStatus: <i>Пост опубликован</i>',
                             photo=call.message.photo[0]['file_id'])
        await call.message.edit_caption(call.message.caption + '\nStatus:<i> Опубликовано</i>')
        await bot.send_photo(CHANNEL_ID, caption=call.message.caption,
                             photo=call.message.photo[0]['file_id'])
    else:
