...
# Команда для изменения товара в Списке товаров
...
async def change_id(message: types.Message, state: FSMContext):
    ...
    # Обновляем в Стейте id товара, который выбрал человек
    await state.update_data(id=message.text)

    # Просим у человека что хотим изменить
...
async def change_input(message: types.Message, state: FSMContext):
    ...
    # Обновляем в Стейте id пункта изминения товара
...
async def change_value(message: types.Message, state: FSMContext):
    ...
    # Обновляем в Стейте на какую информацию человек хочет изменить
    await state.update_data(value=message.text)

    # Достаем всю полученую информацию от человека
...
# Ловим фото и меняем его photo_id в базе
