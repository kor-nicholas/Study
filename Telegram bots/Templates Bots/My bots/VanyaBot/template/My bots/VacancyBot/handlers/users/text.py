...
from keyboards.default.reply import start, back
from keyboards.inline.inline import feedback

from aiogram.dispatcher import FSMContext
from states.state import *

from sql.sql_db import SQL_people, SQL_vacancy, SQL_category
...
async def bot_echo(message: types.Message):
    ...
    if message.text == 'Вакансии':
        ...
        # Достаем из базы все категории
        sql_object = SQL_category(db_sql)
        categorys = sql_object.select_all_category()
        sql_object.sql_close()

        # category[0] - id; category[1] - name;

        categorys_murkup = InlineKeyboardMarkup()

        for category in categorys:
            categorys_murkup.add(InlineKeyboardButton(f'{category[1]}', callback_data=f'{category[1]}|'))

        await message.answer("Выберите категорию: ", reply_markup=categorys_murkup)
        ...
        await message.answer("Раздел О нас", reply_markup=back)

    elif message.text == 'Контакты':
        await message.answer("Сообщение для кнопки поддержки", reply_markup=feedback)
        await message.answer("Сообщение для кнопки Назад", reply_markup=back)
